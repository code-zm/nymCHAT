...
class CryptoUtils:
    ...
    def _decrypt_data(self, encrypted_data):
        """Decrypt AES-256-GCM encrypted data that was stored as a Base64 string."""
        try:
            encrypted_data = base64.b64decode(encrypted_data)  # 🔄 Decode from Base64 to bytes
            salt, iv, tag, ciphertext = encrypted_data[:16], encrypted_data[16:28], encrypted_data[28:44], encrypted_data[44:]
            ...
            # Convert to UTF-8 string if possible, otherwise return bytes
            return decrypted_data.decode('utf-8') if decrypted_data.isascii() else decrypted_data

        except Exception as e:
            logger.error(f"decryptData - error :( | {e}")
            return None

    def _encrypt_data(self, plaintext):
        """Encrypt general data (not private keys) using AES-256-GCM."""
        if isinstance(plaintext, str):
            plaintext = plaintext.encode()  # Ensure bytes input
        ...
        # Encode everything to Base64 so it can be stored in the DB as text
        return base64.b64encode(salt + iv + encryptor.tag + ciphertext).decode()
